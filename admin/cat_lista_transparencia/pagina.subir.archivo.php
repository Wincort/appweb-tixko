...
$sql1="SELECT * FROM cat_lista_transparencia where id = '$IdRegistro' limit 1";
$resReg = mysql_query ($sql1);
if($resReg){
	$rowReg=mysql_fetch_array($resReg);
	$LigaTipoArchivo=$rowReg['dirRegistroPagina'];
	$TipoPagina=$rowReg['pagina'];
}

if($tipo=="AUDIO")  $ruta_files=$LigaTipoArchivo.'audio/';
if($tipo=="IMAGEN") $ruta_files=$LigaTipoArchivo.'imagen/';
if($tipo=="DOCTO")  $ruta_files=$LigaTipoArchivo.'documentos/';
if($tipo=="VIDEO")  $ruta_files=$LigaTipoArchivo.'video/';

$MaximaCarga=ini_get('upload_max_filesize').'B';

if($opcion=='UPD'){
	$id = intval($_GET['id']);
	$query="SELECT * FROM cat_lista_transparencia_detalles where IdRegistroDetalle = '$id' limit 1";
	...
	if($opcion=='SAVE'){
		...
		$mes=$_POST["mes"];
		$urlfile=$_FILES["archivo"]; //Aqui se suben tanto las fotos como los videos
		$archivo_valido = true;
		$archivo=$_POST["hiurl"];
		...
		if($archivo_valido){
			if(!empty($_FILES['archivo']['name'])){
				...
				if (uploadFileWithNewName($urlfile,$NombreArchivoSaneado)){
					...
					$db -> begin();
					if($id!=0){
						$Sql="update cat_lista_transparencia_detalles 
								   set titulo='$titulo',
								   publicar='$publicar',
								   archivo='$archivo',
								   tipo='$tipo',
						...
						$Sql="insert into cat_lista_transparencia_detalles 
...
<script language="javascript">
	...
	let MostrarMes = (TipoPagina) => {
		TipoPagina=='CONAC' ? $('#CofigMes').css('display', 'block') : $('#CofigMes').css('display', 'none');
	}
</script>
</body>
</html>
