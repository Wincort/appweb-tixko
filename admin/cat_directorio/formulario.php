...
if($opc=="SAVE")
{
	...
	if($id==0||$id==''){
		$query = "insert into cat_directorio (unidad,nombre,email,telefono,publicar,UM,FUM) values ('$unidad','$nombre','$email','$telefono','$publicar','$IdUsuario',Now())";
		$db -> begin();
	...
	if($id!=0&&$id!=''){
		$res=mysql_query("SELECT * FROM cat_directorio where Id='$id'");
		if($res){
			$row=mysql_fetch_array($res);
			$unidad=utf8_decode($row['unidad']);
    ...
    <fieldset style="border:#CCCCCC solid 2px;">
        ...
        <tr>
			<td>
				...
				<br /><input type="text" id="unidad" name="unidad" value="<?=$unidad?>" />
			</td>
		</tr>
		<tr>
			<td>
		...
		<tr>
			<td>
				...
				<br /><input type="text" id="email" name="email" value="<?=$email?>" />
			</td>
		</tr>
		<tr>
			<td>
