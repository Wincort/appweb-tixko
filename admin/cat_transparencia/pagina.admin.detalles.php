...
switch ($opc){
	case 'tb': ?>
		<table cellspacing="0" cellpadding="0" border="0" id="tblImagenes">
			...
			<?php
			$SQL="SELECT * FROM cat_transparencia_detalles where IdRegistro = '$IdRegistro'";
			$res = mysql_query ($SQL);
			$cont = 0;
			if($res){
				if(mysql_num_rows($res) > 0){
					while($row=mysql_fetch_array($res)){
						$tipo=$row['tipo'];
						$archivo=$row['archivo'];
						if($tipo=="IMAGEN"){$ruta_imagen='multimedia/imagen/'.$archivo; $URLVisual='multimedia/imagen/';}
						if($tipo=="AUDIO"){ $ruta_imagen='../imagen/mp3.png'; $URLVisual='multimedia/audio/';	}				
						if($tipo=="VIDEO"){ $ruta_imagen='../imagen/mp4.png'; $URLVisual='multimedia/video/';}
						if($tipo=="DOCTO"){$ruta_imagen='../imagen/pdf.png'; $URLVisual='multimedia/doctos/';} /*$URLVisual='../ViewerJS/#../cat_transparencia/multimedia/doctos/';}*/
						...
						<tr>
							<td>
								...
								<div class="aciones">
									...
									<!--<a onclick="return GB_showCenter('Ver Archivo', this.href, 500, 650);" href="<?=$LigaArchivo?>">Ver</a>&nbsp;-->
	...
	case 'del':
		$id=$_POST['id'];
		$Sql="delete from cat_transparencia_detalles where IdRegistroDetalle=$id";
		$db -> begin();
