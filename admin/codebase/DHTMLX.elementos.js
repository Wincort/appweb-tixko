...
/*	DHTMLX_MESSAGE*/
	...
	function aviso(mensaje,tipo){/*<img src='../images/alert_small.gif' style='float:left;'> */
		dhtmlx.message({type:tipo,type:"cssAviso",text:"<div style='margin-top:0px;'><img width='22' src='../admin/images/alert_small.png' style='float:left;margin-left:1px;'/><b> "+mensaje+"</b></div>",  expire:2000 });
	}
	...
	function mensajeFuncion(mensaje,href,titulo){
	  	dhtmlx.alert({title:titulo,	ok:"OK",text: mensaje, callback:function(){
			if(href!=''){
	...
	function confirmar(mensaje,href){
		var x=dhtmlx.confirm("<img src='../admin/images/alert_medium.png' style='float:left;margin-left:30px;'> <div style='font-weight:bold;margin-top:10px;text-align:left;width:350px'>Need rich <a target='blank' href='"+href+"'>"+mensaje+".</a></div>");
		if(x==true){ alert("entro");} else{ alert("");}
		}
		
	function confirmarFuncionHref(mensaje,funcion,href){
		if(href!=''){ href = href;} else{href='';}
		var x=dhtmlx.confirm("<img src='../images/alert_medium.png' style='float:left;margin-left:30px;'> <div style='font-weight:bold;margin-top:10px;text-align:left;width:350px'>  <a target='blank' href='"+href+"'>"+mensaje+".</a></div>");
...
function TestConfirm1(mensaje,titulo,href){
			
			dhtmlx.confirm({title:titulo,ok:"Ok", cancel:"<a onclick='window.close();' target='_self' >Cerrar</a>",	text:mensaje,	callback:function(result){
	...
	function TestConfirm(mensaje,titulo,href){
			dhtmlx.confirm("Test confirm", function(result){
			dhtmlx.confirm({title:"Custom title",ok:"Yes", cancel:"No",	text:"Result: "+result,	callback:function(result){
