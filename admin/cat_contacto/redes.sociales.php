...
<head>
    ...
    <script type="text/javascript">
        ...
        dhtmlxEvent(window,"load",function(){                                         
            ...
            masterGrid.setHeader("Nombre,Información");                           
            masterGrid.setColumnIds("nombre,descripcion");                         
            //masterGrid.setInitWidths("150,150,100");                                
            masterGrid.setColAlign("left,left");                            
            masterGrid.setColTypes("ro,ro");                                 
            masterGrid.setColSorting("icon,str,str");                               
            masterGrid.attachHeader("#select_filter,#text_filter");      
			masterGrid.attachEvent("onRowSelect",function(id){
				progressOn(false);
				myCellB.attachURL("redes.sociales.subir.archivo.php?opc=UPD&tipo=IMAGEN&id="+id);
                myCellB.expand();
                myCellA.collapse();
			});
			masterGrid.setSkin("dhx_web");
            masterGrid.init();                                                    
            masterGrid.load("data/redes.xml.php");

			
            var dpg = new dataProcessor("data/redes.datos.php");                          //inits dataProcessor
		...
		function recargarGrid(){
			masterGrid.clearAndLoad("data/redes.xml.php");
		}
